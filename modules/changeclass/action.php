...
//include_once( 'extension/expchangeclass/classes/functions.php' );
...
elseif ( $Module->isCurrentAction( 'ModifyObject' )  )
{
    ...
    // Every one of these was read conditionally and used unconditionally, so a
    // request missing any of them reached convertObject() with an undefined
    // variable. Missing input is refused here instead.
    if ( !$http->hasPostVariable( 'SourceObjectID' ) ||
         !$http->hasPostVariable( 'DestinationClassID' ) ||
         !$http->hasPostVariable( 'SourceAttribute' ) )
    {
        eZDebug::writeError( 'Conversion requested without the required post variables', __FILE__ );
        return $Module->handleError( eZError::KERNEL_NOT_AVAILABLE, 'kernel' );
    }

    $sourceObjectID     = (int) $http->postVariable( 'SourceObjectID' );
    $destinationClassID = $http->postVariable( 'DestinationClassID' );
    $mapping            = $http->postVariable( 'SourceAttribute' );
    $sourceNodeID       = $http->hasPostVariable( 'SourceNodeID' )
                        ? (int) $http->postVariable( 'SourceNodeID' ) : 2;

    // The result was discarded, so a refused or rolled back conversion still
    // rendered the success page and told the operator the content had changed.
    $conversionError = null;
    $converted = conversionFunctions::convertObject( $sourceObjectID, $destinationClassID,
                                                     $mapping, $conversionError );

    if ( !$converted )
    {
        eZDebug::writeError( "Conversion of object $sourceObjectID failed: " .
                             (string) $conversionError, __FILE__ );
    }
...
elseif ( $Module->isCurrentAction( 'CreateObject' ) )
{
	...
	eZDebug::writeError( 'Operation temporary not available' );
	return $module->handleError( eZError::KERNEL_NOT_AVAILABLE, 'kernel' );
	/*
    if ( $http->hasPostVariable( 'SourceObjectID' ) )
    {
...
if ( $http->hasPostVariable( 'GenerateConsoleParameters' ) )
{
    ...
    // So the page can say plainly whether the content actually changed.
    $tpl->setVariable( 'converted', isset( $converted ) ? $converted : true );
    $tpl->setVariable( 'conversion_error', isset( $conversionError ) ? $conversionError : null );
